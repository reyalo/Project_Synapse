...
// {
//   int n = s.size(), m = 1 + n >> 1;
//   fri(0, m) if (s[i] != s[n - i - 1]) return 0;
...
void build(int l, int r, vi const &a, vi &p, int d)
{
  if (l <= r)
  {
    int pos = max_element(a.bn + l, a.bn + r + 1) - a.bn;
  }
}
...
ll fn(ll k, ll n)
{
  ...
  ll res = (nk * (nk + 1) + nk * (nk - 1) - (nk - l) * (nk - 1 - l)) / 2;
  return res;
}

/* primes List*/
#define m 1000010 // it provide <m*m all primes
vi primes;
bool siv[m];
void sieve()
{
  primes.pb(2);
  fric(ll(3), m, 2) if (!siv[i]) frjc(i * i, m, i + i) siv[j] = 1; // explain in cp_necessary
  fric(ll(3), m, 2) if (!siv[i]) primes.pb(i);
}
ll fact(ll n)
{
  ll ans = 1;
