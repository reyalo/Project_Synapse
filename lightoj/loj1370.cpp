...
void phi_1_to_n(int n) {
    ...
    fri(2,n+1)
      if(phi[i]==i)frjc(i,n+1,i)phi[j] -= phi[j] / i;
...
void phi_1(ll n){
  fri(0,n){
    if(mm[phi[i]]==0)mm[phi[i]]=i;
    else mm[phi[i]]<i?:mm[phi[i]]=i;
  }
  rfi(1000001,1){
    if(mm[i]==0)mm[i]=mm[i+1];
  }
}
...
void solve()
{
  ...
  // }
  // frit(mm)cout<<i->fi<<sp<<i->se<<endl;
  ll n;
  cin>>n;
  vi a(n);
  fri(0,n)cin>>a[i];
  ...
  fri(0,n){
   auto it=lower_bound(vv.bn,vv.en,a[i]);
   cout<<it->fi<<sp<<it->se<<el;
  }
  cout<<ans<<" Xukha";
  cout<<el;
