...
{
    if (b == e) {  // when leaf node
        tree[node] = arr[b];
        return;
    }
    int Left = (node<<1), Right = (node<<1 | 1), mid = (b+e)>>1;

    init(Left, b, mid);          // left -> left node no. b~mid left node range
    init(Right, mid + 1, e);     // right -> right node no. mid+1~e right node range

    // ei line e asbe first init and 2nd init complete houar por

    // tree[node] is the current node and we can save any value to this
}

int query_(int node, int b, int e, int i, int j,int carry=0)
{
    if (i > e || j < b)
    ...
    // method 1:
    // if(i<=mid) return query_(Left, b, mid, i, j,carry+prop[node]);
    // else return query_(Right, mid + 1, e, i, j, carry + prop[node]);

    // method 2:
    int p1= query_(Left, b, mid, i, j,carry+prop[node]);
    int p2= query_(Right, mid + 1, e, i, j, carry + prop[node]);

    return p1+p2;
}

void update_(int node, int b, int e, int i,int j)
{   
  // please remember b and e is the current node range
  // i~j is the given range which to be update


    if (i > e || j < b)
        return; //
    if (b >= i && e <= j) {  // if currents node range is inside the given range then it called relevant node
        tree[node]+=1;
        prop[node]+=1;        // we can go back from this because all successor of current node achive prop[node] value
        return;
    }
    int Left = (node<<1), Right = (node<<1 | 1), mid = (b+e)>>1;

    update_(Left, b, mid, i, j);        // left -> left node no. b~mid left node range
    update_(Right, mid + 1, e, i, j);   // right -> right node no. mid+1~e right node range
}
...
void solve()
{
  ...
  frri(q){
        ...
        case 'Q':
            cin >> x;
            cout<<(arr[x]^(query_(1, 1, n, x,x))&1)<<el;
            // frri(1,8)cout<<prop[i];
            break;
        }
  }
}
