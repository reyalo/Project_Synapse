...
void solve()
{
  double a,aa,b,bb,c,cc,r1,r2,r3;
  cin>>a>>aa>>b>>bb>>c>>cc;
  r1=sqrt((b-a)*(b-a)+(bb-aa)*(bb-aa));
  r2=sqrt((c-a)*(c-a)+(cc-aa)*(cc-aa));
  r3=sqrt((c-b)*(c-b)+(cc-bb)*(cc-bb));
  
  double theta=angle(r3,r1,r2);
  
  cout<< fixed << setprecision(6) <<r1*theta<<el;
}
