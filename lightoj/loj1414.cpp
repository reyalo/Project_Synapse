...
void solve()
{
  ll d,dd,y,yy,c,cc,ccc;
  string m,mm;
  char dm;
  cin>>m>>d>>dm>>y>>mm>>dd>>dm>>yy;
  //  segment                                       start         end
  // it is done by segment done method _______________|____________|____________
  if(!(m=="January" || m=="February"))y++;
  if(mm=="January"||(mm=="February"&&dd<29))yy--;

  // assume first year include february and last year included february
  c=yy/4-(y-1)/4;
  cc=yy/100-(y-1)/100;
  ccc=yy/400-(y-1)/400;
