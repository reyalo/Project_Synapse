...
ll prop[mx * 3];
// ii tree2[mx*3];
int min_idx(int a,int b){return arr[a]<arr[b]?a:b;}
void init(int node, int b, int e)
{
    ...
    // if you want to calculate internal node value when backward

    tree[node] = min_idx(tree[Left],tree[Right]);
...
int query(int node, int b, int e, int i, int j)
{
    ...
    // etake abar dui vaag korbo. eg. 4=2+2,3=2+1 (4 = current node successor,2= left node successor,1 = right node successor,)
    ...
    return min_idx(p1,p2); //বাম এবং ডান পাশের যোগফল  // min(p1,p2) keno tree[node] save kore ni?? karon eta just query
...
void update(int node, int b, int e, int i, int newvalue)
{
    ...
    if (b >= i && e <= i) {                                             //রিলেভেন্ট সেগমেন্ট
        tree[node] = newvalue?tree[node]+newvalue:0;                    //নতুন মান বসিয়ে দিলাম
        return;
    }
    int Left = (node<<1), Right = (node<<1 | 1), mid = (b+e)>>1;
...
ll query_lazy_sum(int node, int cl, int cr, int gl, int gr,ll carry=0)
{
    if (gl > cr || gr < cl)
        ...
        // return 0; //বাইরে চলে গিয়েছে
    if (cl >= gl && cr <= gr){
        // cout<<b<<sp<<cr<<sp<<node;
        // deb(carry);
        return tree[node]+(cr-cl+1)*carry; //রিলেভেন্ট সেগমেন্ট
    }
...
void solve()
{
  ...
  // aa.erase(aa.bn+1,aa.bn+2);
  // frri(aa.sz)cout<<aa[i]<<sp;
  // cout<<el;
  frri(q){
    char cc;
    cin>>cc;
    if(cc=='c'){
      cin>>x;
      if(x>aa.sz){cout<<"none"<<el;continue;}
      x--;
      cout<<aa[x]<<el;
      aa.erase(aa.bn+x,aa.bn+x+1);
    }else{
      cin>>x;
      aa.pb(x);
    }
